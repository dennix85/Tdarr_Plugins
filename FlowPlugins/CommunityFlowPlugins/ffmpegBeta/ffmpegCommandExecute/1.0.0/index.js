...
// ============================================================================
// FILTER MERGING FUNCTIONS
// ============================================================================

// Merge all -vf (video filters) into a single filter chain
var mergeVideoFilters = function (outputArgs, args) {
    var videoFilters = [];
    var newOutputArgs = [];
    var skipNext = false;
    
    // Extract all -vf filters
    for (var i = 0; i < outputArgs.length; i++) {
        ...
        if (outputArgs[i] === '-vf') {
            if (i + 1 < outputArgs.length) {
                videoFilters.push(outputArgs[i + 1]);
                skipNext = true;
                args.jobLog("Found video filter: ".concat(outputArgs[i + 1]));
            }
        } else {
            newOutputArgs.push(outputArgs[i]);
        }
    }
    
    // Merge if multiple filters found
    if (videoFilters.length > 1) {
        args.jobLog("Merging ".concat(videoFilters.length, " video filters into single chain"));
        var mergedFilter = videoFilters.join(',');
        newOutputArgs.push('-vf', mergedFilter);
        args.jobLog("Merged video filter chain: ".concat(mergedFilter));
    } else if (videoFilters.length === 1) {
        newOutputArgs.push('-vf', videoFilters[0]);
    }
    
    return newOutputArgs;
};

// Merge all -af (audio filters) into a single filter chain
var mergeAudioFilters = function (outputArgs, args) {
    var audioFilters = [];
    var newOutputArgs = [];
    var skipNext = false;
    
    // Extract all -af filters
    for (var i = 0; i < outputArgs.length; i++) {
        if (skipNext) {
            skipNext = false;
            continue;
        ...
        if (outputArgs[i] === '-af') {
            if (i + 1 < outputArgs.length) {
                audioFilters.push(outputArgs[i + 1]);
                skipNext = true;
                args.jobLog("Found audio filter: ".concat(outputArgs[i + 1]));
            }
        } else {
            newOutputArgs.push(outputArgs[i]);
        }
    ...
    if (audioFilters.length > 1) {
        args.jobLog("Merging ".concat(audioFilters.length, " audio filters into single chain"));
        var mergedFilter = audioFilters.join(',');
        newOutputArgs.push('-af', mergedFilter);
        args.jobLog("Merged audio filter chain: ".concat(mergedFilter));
    } else if (audioFilters.length === 1) {
        newOutputArgs.push('-af', audioFilters[0]);
    }
    
    return newOutputArgs;
};

// Validate for common conflicts and warn user
var validateOutputArgs = function (outputArgs, stream, args) {
    var hasCrf = false;
    var hasBitrate = false;
    
    for (var i = 0; i < outputArgs.length; i++) {
        if (outputArgs[i] === '-crf' || outputArgs[i] === '-qp' || outputArgs[i] === '-global_quality') {
            hasCrf = true;
        }
        if (outputArgs[i].includes('-b:v:') || outputArgs[i].includes('-b:V:')) {
            hasBitrate = true;
        }
    }
    
    if (hasCrf && hasBitrate) {
        args.jobLog("WARNING: Stream ".concat(stream.index, " has both CRF/QP and bitrate set!"));
        args.jobLog('This may cause unexpected behavior. FFmpeg typically ignores bitrate when CRF is set.');
        args.jobLog('Consider using only one rate control method.');
    }
};
        ...
        switch (_b.label) {
            case 0:
                ...
                _loop_1 = function (i) {
                    ...
                    // ============================================================================
                    // NEW: Merge filters and validate before adding to cliArgs
                    // ============================================================================
                    if (stream.codec_type === 'video') {
                        stream.outputArgs = mergeVideoFilters(stream.outputArgs, args);
                    }
                    
                    if (stream.codec_type === 'audio') {
                        stream.outputArgs = mergeAudioFilters(stream.outputArgs, args);
                    }
                    
                    // Validate for common conflicts
                    validateOutputArgs(stream.outputArgs, stream, args);

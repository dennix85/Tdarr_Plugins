...
const details = () => ({
    ...
    description: 'Intelligently adds tags to Radarr, Sonarr, and Whisparr content. Supports multiple instances, automatic content detection via IMDB ID and filename parsing, and handles tag creation when tags do not yet exist.',
    style: {
        borderColor: '#FF6B35',
    ...
    tags: 'sonarr,radarr,whisparr,tags,tagging,multi-instance,automation',
    isStartPlugin: false,
    pType: '',
    ...
    inputs: [
        ...
        // === TAG CONFIGURATION ===
        {
            ...
            defaultValue: 'tdarr',
            inputUI: { type: 'text' },
            tooltip: 'The tag to add to content (e.g., "tdarr", "processed", "transcoded"). Will be created if it does not already exist.',
        },
    ...
    outputs: [
        {
            number: 1,
            tooltip: '✅ Tag added successfully to content',
        },
        {
            number: 2,
            tooltip: '🏷️ Tag already existed on content - no changes made',
        },
        {
            number: 3,
            tooltip: '❌ Failed to add tag - check configuration, API keys, and connectivity',
        },
    ],
});
        ...
        if (apiKey && host) {
            ...
            instances.push({
                ...
                headers: {
                    ...
                    'User-Agent': 'DeNiX-Smart-Tag-Manager/1.0.0',
                },
            });
...
// Look up content ID from an instance using IMDB ID or filename parsing
...
const getOrCreateTag = (instance, tagName, inputs, logger) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const tagsUrl = `${instance.host}/api/v3/tag`;

        logger.debug(`Getting all tags from ${instance.name}...`);
        ...
        if (!Array.isArray(allTags)) {
            logger.error('Invalid tags response from API - not an array');
            return null;
        }

        logger.debug(`Retrieved ${allTags.length} existing tags from ${instance.name}`);

        // Check if tag already exists
        const existingTag = allTags.find(tag =>
            tag.label && tag.label.toLowerCase() === tagName.toLowerCase()
        );

        if (existingTag) {
            logger.tags(`Found existing tag: "${tagName}" (ID: ${existingTag.id})`);
            return existingTag.id;
        }

        // Create the new tag
        logger.tags(`Tag "${tagName}" not found - creating it now...`);

        const createResponse = yield makeApiRequest(
            'post', tagsUrl, { label: tagName },
            instance.headers, inputs.request_timeout, inputs.max_retries, logger
        );
        ...
        if (newTag && newTag.id) {
            logger.success(`Created new tag: "${tagName}" (ID: ${newTag.id})`);
            return newTag.id;
        }

        logger.error(`Tag creation response did not contain an ID`);
        return null;

    } catch (error) {
        logger.error(`Failed to get or create tag: ${error.message}`);
        return null;
    }
});

// Apply tag to content in the given instance
const applyTagToContent = (instance, contentId, tagName, inputs, logger) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // Step 1: Get current content data
        const contentResponse = yield makeApiRequest(
            'get', contentUrl, null,
            instance.headers, inputs.request_timeout, inputs.max_retries, logger
        );
        ...
        logger.debug(`Retrieved content: "${contentData.title}"`);
        logger.debug(`Current tags on content: [${(contentData.tags || []).join(', ')}]`);

        // Step 2: Get or create the tag
        const tagId = yield getOrCreateTag(instance, tagName, inputs, logger);

        if (tagId === null) {
            return { success: false, error: 'Failed to get or create tag' };
        }

        logger.debug(`Tag "${tagName}" has ID: ${tagId}`);

        // Step 3: Check if content already has this tag
        const currentTags = contentData.tags || [];
        const hasTag = currentTags.includes(tagId);

        if (hasTag) {
            logger.tags(`Content "${contentData.title}" already has tag "${tagName}"`);
            return {
                success: true,
                alreadyTagged: true,
        ...
        // Step 4: Add tag and PUT updated content back
        const updatedContent = Object.assign({}, contentData, {
            tags: currentTags.concat([tagId]),
        });

        logger.tags(`Adding tag "${tagName}" to "${contentData.title}"...`);

        yield makeApiRequest(
            'put', contentUrl, updatedContent,
            instance.headers, inputs.request_timeout, inputs.max_retries, logger
        );

        logger.success(`Tag "${tagName}" successfully added to "${contentData.title}"`);
...
const plugin = (args) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const tagName = String(args.inputs.tagName || 'tdarr').trim();

        logger.section('DeNiX Smart Tag Manager: Multi-Instance *ARR Tagging');
        ...
        logger.tags(`Tag to apply: "${tagName}"`);

        if (!tagName) {
            logger.error('No tag name specified - please set the Tag Name input');
        ...
        // -----------------------------------------------
        // STEP 2: FIND CONTENT AND APPLY TAG
        // -----------------------------------------------

        logger.subsection('Step 2: Content lookup and tag operation');
        ...
        for (let i = 0; i < instances.length; i++) {
            ...
            // Look up the file in this instance
            ...
            logger.success(`Found content (ID: ${contentId}) in ${instance.name} - applying tag...`);

            // Apply the tag
            const result = yield applyTagToContent(instance, contentId, tagName, args.inputs, logger);

            if (!result.success) {
                stats.errors.push(`${instance.name}: ${result.error}`);
                logger.error(`Tag operation failed on ${instance.name}: ${result.error}`);
                continue;
            }
            ...
            logger.subsection('Step 3: Result');

            const totalTime = Date.now() - startTime;
            const serviceIcon = instance.type === 'sonarr' ? '📺' : instance.type === 'radarr' ? '🎬' : '🔞';

            if (result.alreadyTagged) {
                logger.tags(`${serviceIcon} "${result.contentTitle}" already had tag "${tagName}" - nothing to do`);
                logger.info(`📊 Instance: ${instance.name} | Content: ${result.contentTitle} | Tag ID: ${result.tagId}`);
            ...
            logger.success(`🎊 ${serviceIcon} Tag "${tagName}" added to "${result.contentTitle}" in ${totalTime}ms`);
            logger.info(`📊 Instance: ${instance.name} | Content: ${result.contentTitle} | Tag ID: ${result.tagId}`);
        ...
        logger.section('No Successful Tag Operation');
        ...
        if (stats.foundInstances === 0) {
            ...
            logger.info('File was found but tagging failed:');
            logger.extended('  • Verify API keys have write permissions');

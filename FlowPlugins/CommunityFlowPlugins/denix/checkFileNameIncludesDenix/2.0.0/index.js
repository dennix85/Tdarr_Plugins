...
const plugin = (args) => __awaiter(void 0, void 0, void 0, function* () {
    try {
		...
		// SKIP MODE: Takes priority over everything
		if (skipMode) {
			if (matchResult.isMatch) {
				// File already has the tag/pattern -> already processed -> skip it
				logger.warn('⏭️ Skip mode activated - file already processed (match found)');
				...
				outputNumber = 2;
				reason = 'Skip mode: File already processed (match found)';
				logger.success(`File set to original: ${args.originalLibraryFile._id}`);
			} else {
				// No match found -> file needs processing -> continue
				logger.info('✅ Skip mode: File needs processing (no match found)');
				outputNumber = 1;
				reason = 'Skip mode: File needs processing (no match)';
			}
		} 
		// NORMAL MODE: Standard matching logic
		else {
			...
			outputNumber = finalMatch ? 1 : 2;
			reason = invertMatch 
				? (finalMatch ? 'Inverted: No match → Proceed' : 'Inverted: Match found → Skip')
